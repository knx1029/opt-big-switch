...
int main(int argc, char* argv[]) {
  ...
  f_out.open(argv[3]);
  sscanf(argv[4], "%d", &numProc);
  sscanf(argv[5], "%d", &kthProc);
  ...
  f_out << N_PATH << endl;
  int paths_per_proc = (N_PATH  + numProc - 1)/ numProc;
  int start_path = paths_per_proc * kthProc;
  int end_path = min(paths_per_proc * (kthProc + 1), N_PATH);
